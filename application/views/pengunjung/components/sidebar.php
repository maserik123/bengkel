<nav class="pcoded-navbar">
    <div class="sidebar_toggle"><a href="#"><i class="icon-close icons"></i></a></div>
    <div class="pcoded-inner-navbar main-menu">
        <div class="pcoded-navigation-label" data-i18n="nav.category.navigation">Menu Navigation</div>
        <ul class="pcoded-item pcoded-left-item">
            <li class="active">
                <a href="<?php echo base_url() ?>" class="waves-effect waves-dark">
                    <span class="pcoded-micon"><i class="ti-home"></i><b>D</b></span>
                    <span class="pcoded-mtext" data-i18n="nav.dash.main">Beranda</span>
                    <span class="pcoded-mcaret"></span>
                </a>
            </li>
            <li class="">
                <a href="<?php echo base_url('cari') ?>" class="waves-effect waves-dark">
                    <span class="pcoded-micon"><i class="ti-search"></i><b>FC</b></span>
                    <span class="pcoded-mtext" data-i18n="nav.content.main">Cari Bengkel</span>
                    <span class="pcoded-mcaret"></span>
                </a>
            </li>
        </ul>

        <div class="pcoded-navigation-label" data-i18n="nav.category.navigation">Bengkel</div>
        <ul class="pcoded-item pcoded-left-item">
            <div class="pcoded-navigation-label" style="color: black;" data-i18n="nav.category.navigation">
                <p>Anda memiliki bengkel ? Bergabunglah bersama kami agar orang-orang dapat menemukan bengkel anda.</p>
            </div>

            <li class="">
                <a href="<?php echo base_url('owner') ?>" class="btn btn-default">
                    <span class="pcoded-micon"><i class="ti-user"></i><b>FC</b></span>
                    <span class="pcoded-mtext" data-i18n="nav.form-components.main">Daftarkan Bengkel</span>
                    <span class="pcoded-mcaret"></span>
                </a>
            </li>
        </ul>

    </div>
</nav>